...
export default class Color {
  ...
  initHsb = () => {
    ...
    const HSV = this.color.toHsvString();
    let left = HSV.indexOf('(') + 1
    let right = HSV.indexOf(')')
    let hsvString = HSV.slice(left,right).split(',')
    this.hueValue = Number(hsvString[0]);
    this.saturationValue = parseInt(hsvString[1]) / 100;
    this.brightnessValue = parseInt(hsvString[2]) / 100;
  };
